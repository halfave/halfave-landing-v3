      ...
      <div className="rp-root">
        ...
        <div className="rp-body">
          ...
          <div style={{
            ...
            {/* Permalink + borough ZIP rankings */}
            {(() => {
              const bldg = (window as any).__halfaveBldg;
              const slug = bldg?.building?.slug ?? null;
              const boroughNum = String(bldg?.building?.borough ?? building.borough ?? '');
              const boroughSlug: Record<string,string> = { '1':'manhattan','2':'bronx','3':'brooklyn','4':'queens','5':'staten-island' };
              const boroughName: Record<string,string> = { '1':'Manhattan','2':'Bronx','3':'Brooklyn','4':'Queens','5':'Staten Island' };
              const bSlug = boroughSlug[boroughNum];
              const bName = boroughName[boroughNum];
              return (
                <div style={{ marginBottom: 12, display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center' }}>
                  {slug && (
                    <span style={{ fontSize: 11 }}>
                      🔗 Permalink:{' '}
                      <a
                        href={`https://halfave.co/buildings/${slug}`}
                        style={{ color: 'var(--navy)', textDecoration: 'underline', fontFamily: "'DM Mono', monospace" }}
                      >
                        halfave.co/buildings/{slug}
                      </a>
                    </span>
                  )}
                  {bSlug && (
                    <span style={{ fontSize: 11 }}>
                      📊{' '}
                      <a
                        href={`/zip-rankings/${bSlug}`}
                        style={{ color: 'var(--navy)', textDecoration: 'underline' }}
                        onClick={e => { e.preventDefault(); window.history.pushState({}, '', `/zip-rankings/${bSlug}`); window.dispatchEvent(new PopStateEvent('popstate')); }}
                      >
                        Top ZIP codes in {bName} by health score →
                      </a>
                    </span>
                  )}
                </div>
